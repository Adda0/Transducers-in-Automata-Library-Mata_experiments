...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " mata_file.mata TEMP_DIR" << std::endl;
        return 1;
    }
    ...
    // for (auto& nft: nfts) {
        ...
        // Store expanded file in a temporary folder.
        std::ofstream result;
        result.open(std::filesystem::path(argv[2]) / std::string("expanded_before_composition.mata"));
        // std::cout << std::filesystem::path(argv[2]) / std::string("expanded_before_composition.mata") << "\n";
        lhs_synced.print_to_mata(result);
        rhs_synced.print_to_mata(result);
        result.close();
